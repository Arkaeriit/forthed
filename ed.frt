...
false value ed-default-prompt-state
...
: ed-init ( -- ) list-init to ed-lst
    ...
    ed-default-prompt-state to ed-prompt-enabled
...
( Execute the c command, which is equivalent to d + -a.)
: ed-command-c ( range -- ) ed-range-action ed-error-command
    ed-command-d
    list-init ed-current-line 1- over nlist-append
    ed-command-a ;
...
:noname ( -- ) ( TODO : prompt ) begin
        ed-prompt-enabled ed-mode ed-mode-command = and
            if ed-prompt str-buff-get type then
